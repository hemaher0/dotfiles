...
function Resolve-Msys2Command {
    ...
    $Candidate = Join-Path "C:\msys64\usr\bin" "$Name.exe"
    if (Test-Path -Path $Candidate -PathType Leaf) {
        return $Candidate
    }

    foreach ($Prefix in @("ucrt64", "mingw64", "clang64", "clangarm64")) {
        $Candidate = Join-Path "C:\msys64\$Prefix\bin" "$Name.exe"
        if (Test-Path -Path $Candidate -PathType Leaf) {
            return $Candidate
        }
    }
...
function Get-NvimPluginRows {
    ...
    foreach ($Plugin in $Plugins) {
        ...
        if (Test-Path -Path $Path -PathType Container) {
            ...
            $Missing += 1
            $Rows += New-ComponentRow "missing" "plugin" "nvim" "local" $Plugin.Id $Plugin.Name "lazy.nvim" "managed by plugin-nvim" "-" "-" "install" $Plugin.Id
        }
    }
...
function Get-ComponentRows {
    ...
    $Rows += New-Msys2CommandRow "package-msys2" "MSYS2 zsh environment" @("zsh", "bash")
...
function Invoke-ComponentAction {
    ...
    switch -Regex ($ComponentId) {
        ...
        "^plugin-zsh" { Invoke-ZshPluginAction $ActionName; break }
